...
class PPQ_Admin_AI_Generation {
	...
	public function enqueue_scripts( $hook ) {
		...
		wp_localize_script(
			...
			[
				...
				'maxFileSize'       => PPQ_File_Processor::get_max_file_size(),
				'maxFileSizeFormatted' => size_format( PPQ_File_Processor::get_max_file_size() ),
				'supportedTypes'    => PPQ_File_Processor::get_supported_types(),
				'strings'           => [
	...
	public function render_panel( $bank_id ) {
		...
		$api_status = PPQ_AI_Service::get_api_key_status( $user_id );
				...
				<label class="ppq-ai-label">
					...
					<span class="ppq-ai-label-hint">
						...
						printf(
							...
							esc_html( size_format( PPQ_File_Processor::get_max_file_size() ) )
						);
						?>
					</span>
				</label>
	...
	public function ajax_generate_questions() {
		...
		$types             = isset( $_POST['types'] ) ? array_map( 'sanitize_key', (array) $_POST['types'] ) : [ 'mc' ];
		$difficulty        = isset( $_POST['difficulty'] ) ? array_map( 'sanitize_key', (array) $_POST['difficulty'] ) : [ 'medium' ];
		$answer_count      = isset( $_POST['answer_count'] ) ? absint( $_POST['answer_count'] ) : 4;
		$generate_feedback = isset( $_POST['generate_feedback'] ) ? (bool) absint( $_POST['generate_feedback'] ) : true;
		...
		$ai_service = new PPQ_AI_Service( $api_key );

		// Set user's preferred model if available
		$model = PPQ_AI_Service::get_model_preference( $user_id );
		if ( $model ) {
	...
	public function ajax_upload_file() {
		...
		// Process the file
		$processor = new PPQ_File_Processor();
	...
	private function create_question( $data, $categories, $bank_id ) {
		...
		$question_id = PPQ_Question::create( $question_data );
		...
		$revision_id = PPQ_Question_Revision::create_for_question( $question_id, $revision_data );
