 ...
 *
 * Fires when a user completes any quiz (pass or fail).
 *
 ...
 * @since 1.0.0
 */
class PressPrimer_Quiz_Quiz_Completed extends \Uncanny_Automator\Recipe\Trigger {
	...
	protected function setup_trigger() {
		...
		$this->set_trigger_code( 'PPQ_QUIZ_COMPLETED' );
		...
		$this->set_sentence(
			sprintf(
				...
				esc_attr__( 'A user completes {{a quiz:%1$s}}', 'pressprimer-quiz' ),
				$this->get_trigger_meta()
			)
		);

		$this->set_readable_sentence(
			esc_attr__( 'A user completes {{a quiz}}', 'pressprimer-quiz' )
		);

		// Hook into the quiz completed action.
		$this->add_action( 'pressprimer_quiz_quiz_completed', 10, 2 );
	}
	...
	public function define_tokens( $trigger, $tokens ) {
		...
		$tokens[] = array(
			'tokenId'   => 'PASSED',
			'tokenName' => esc_attr__( 'Passed (Yes/No)', 'pressprimer-quiz' ),
			'tokenType' => 'text',
		);
