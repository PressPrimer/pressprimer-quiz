 ...
 * @since 1.0.0
 */
class PressPrimer_Quiz_Quiz_Failed extends \Uncanny_Automator\Recipe\Trigger {
	...
	protected function setup_trigger() {
		...
		$this->set_trigger_code( 'PPQ_QUIZ_FAILED' );
		...
		$this->set_sentence(
			sprintf(
				...
				esc_attr__( 'A user fails {{a quiz:%1$s}}', 'pressprimer-quiz' ),
				$this->get_trigger_meta()
			)
		);

		$this->set_readable_sentence(
			esc_attr__( 'A user fails {{a quiz}}', 'pressprimer-quiz' )
		);

		// Hook into the quiz failed action.
		$this->add_action( 'pressprimer_quiz_quiz_failed', 10, 2 );
	}
